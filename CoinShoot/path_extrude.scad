...
function getPreRotations(extrudePath, refPt, merge=false, prs = [0]) =
    (len(prs) >= (len(extrudePath))) ? prs :
        getPreRotations(extrudePath=extrudePath, refPt=refPt, merge=merge,
            prs=concat(prs,getNPAngle(p1=project(p1=arrMod(extrudePath,prs,-1) +
                    myRotate(rToS(arrMod(extrudePath,prs,(!merge&&(len(prs)<2))?-1:0) - 
                      arrMod(extrudePath,prs,
                          (!merge&&(len(prs) > len(extrudePath)-2))?-1:-2)),
                        myRotate([0,0,-prs[len(prs)-1]], refPt)),
                n1=arrMod(extrudePath,prs,0) - arrMod(extrudePath,prs,-1),
                c1=extrudePath[len(prs)]),
            c1=extrudePath[len(prs)],
            n1=arrMod(extrudePath,prs,0) - arrMod(extrudePath,prs,-1),
            p2=extrudePath[len(prs)] + 
                myRotate(rToS(arrMod(extrudePath,prs,(!merge&&(len(prs)<2))?0:1) - 
                    arrMod(extrudePath,prs,
                        (!merge&&(len(prs)>len(extrudePath)-2))?0:-1)), refPt))));
...
function makePolyPoints(polyPath, polyForm, polyAngles, merge = false, acc = [], aDone = 0) =
  (aDone >= len(polyPath)) ? acc :
    makePolyPoints(polyPath, polyForm, polyAngles, merge = merge,
        acc=concat(acc,  
            [myTranslate(arrMod(polyPath,acc,0),
                myRotate(rToS(arrMod(polyPath,acc,((!merge) && 
                    (aDone>=(len(polyPath)-1)))?0:1) - 
...
myPath = [ for(t = [0:(360 / 23):359]) [ // trefoil knot
...
//myPath = [ for(t = [0:(360/8):359]) 8 * [cos(t+45),sin(t+45),0] ]; // octagon

myPoints = [ for(t = [0:(360/12):359]) 2 * [cos(t+45),sin(t+45)] ];
...
module path_extrude(exPath = myPath, exShape = myPoints, exRots = [0], merge=false){
    ...
    rawPreRots = getPreRotations(myPath, c3D(myPoints[0]),
               merge=merge, prs = exRots);
    // calculate rotation between last polygon and first polygon
    pp1 = myPath[1];
    p0 = myPath[0];
    pm1 = myPath[len(myPath)-1];
    pm2 = myPath[len(myPath)-2];
    t0 = p0 + myRotate(rToS(pp1-pm1), myRotate([0,0,-rawPreRots[0]], c3D(myPoints[0])));
    tm1 = pm1 + myRotate(rToS(pm1-pm2), 
    ...
    polyPoints = flatten(makePolyPoints(exPath, exShape, preRots, merge=merge));
    if(merge){
...
path_extrude(exRots = [$t * (180)], merge=false);